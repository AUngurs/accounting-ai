describe("Add document", () => {
  ...
  before(() => {
    ...
    cy.createDocument({ id: "001", date: "2025-12-15", amount: "500", comments: "Existing document" });
  ...
  it("ERR25 - Šāds finanšu dokuments jau eksistē", () => {
    ...
    cy.get('input[name="doc_date"]').clear().type("2025-12-15");
    cy.get('input[name="doc_amount"]').clear().type("500");
    cy.get('select[name="partner_id"]').select(1);
  ...
  it("Veiksmīga finanšu dokumenta pievienošana", () => {
    ...
    cy.get('input[name="doc_amount"]').clear().type("500.89");
    cy.get('select[name="partner_id"]').select(1);
    cy.get('textarea[name="doc_comments"]').clear().type("Veiksmīgas pievienošanas dokuments");
    cy.get("button").contains("Pievienot").click();
